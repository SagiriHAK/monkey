...
//    struct timespec req={0, 20 * MILLI_SECOND};//20 ms時間待ち
    ...
    //graph_draw1(ball, monkey, total, req);
    //graph_draw2(ball, monkey, total, req);
    graph_draw3(ball, monkey, total, req);
...
void calc(MASS b[], MASS m[], double g, double r, double ts, int n, int *total)
{
    ...
    do{
        ...
        // b[i].vy = b[i-1].y - g * ts;         //弾丸のy方向速度の計算
        ...
        //m[i].y =  m[i-1].y - g*ts;              //サルのy方向位置の計算
        m[i].y = m[i-1].y + m[i].vy * ts;
